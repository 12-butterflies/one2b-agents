# Jason Daily Stack Template

**HOW TO USE:** Read this file first on every stack question. Fill the variable fields. Run the checklist. Output verbatim. Do NOT generate from memory.

---

## BEFORE ANSWERING — fill these variables:

- [ ] Date and day: _____________ (from `currentDate` memory — state this first)
- [ ] Training today: YES / NO
- [ ] Thymalin day: ___ of 10 (current course Jun 8–17. Check `_daily_log.md`.)
- [ ] Kisspeptin today: YES (Wed/Sat only) / NO
- [ ] State: maintenance / loading / PRN (from `_state.md`)
- [ ] Alcohol last night: YES / NO — if YES, add NAD+ and Glutathione to morning stack. Non-negotiable. Do not wait to be asked.

---

## TRAINING DAY

### Morning — fasted, pre-training (combine into minimum syringes)

| Peptide | SKU | Dose |
|---|---|---|
| CJC-1295 no DAC | (CND2) | 20 IU |
| Ipamorelin | (IP-old) | 20 IU |
| MOTS-c | (MS10) | 38 IU |
| AOD-9604 | (5AD) | 10 IU |
| NAD+ | (NJ500) | 100 IU (1mL = 100mg) — slow push 10-15 min. 2–3x per week only, not daily |
| SS-31 | (2S10) | 5 IU |
| Semax | (XA10) | 10 IU SubQ injectable — separate syringe. Nasal spray = emergency backup only. |

**Needle count: aim for 1–2 syringes. Combine compatible peptides. Incompatible = separate syringe.**

---

### Post-training — SubQ (same syringe if compatible)

| Peptide | SKU | Dose |
|---|---|---|
| BPC-157 + TB-500 blend | (BB20) | 8 IU |
| AOD-9604 | (5AD) | 10 IU — second AOD dose. Same syringe as BB20. |

*This is the second AOD dose for the day. No separate 15:00 shot needed if taken post-workout.*

---

### Post-training — with food (oral or injectable with meal)

| Peptide | SKU | Dose | Note |
|---|---|---|---|
| 5-Amino-1MQ | (50AM) | 50 IU (0.5ml) | Take WITH post-workout meal — NOT fasted. Daily. |

---

### Bedtime — just before sleep, ONE needle

| Peptide | SKU | Dose |
|---|---|---|
| CJC-1295 no DAC | (CND2) | 20 IU |
| Ipamorelin | (IP-old) | 20 IU |
| GHK-Cu | (CU) | 20 IU |
| DSIP | (DS5) | 8 IU |
| Thymalin | (TY10) | 20 IU — **Day ___ of 10** |
| Epitalon | (ET50) | 10 IU — **Day ___ of 10 (course Jun 8–17, same as Thymalin)** |

**Total bedtime: 1 needle. All in one syringe.**

---

## NON-TRAINING DAY

### Morning — fasted (combine into minimum syringes)

| Peptide | SKU | Dose |
|---|---|---|
| SS-31 | (2S10) | 5 IU |
| NAD+ | (NJ500) | 100 IU (1mL = 100mg) — slow push 10-15 min. 2–3x per week only |
| Kisspeptin-10 | (KS10) | **[dose pending]** — **Wed/Sat only** |

*No MOTS-c, no AOD, no BB20, no CJC, no Ipa on non-training day mornings. CJC + Ipa morning dose: training days only. Bedtime: every day.*

---

### With breakfast

| Peptide | SKU | Dose |
|---|---|---|
| 5-Amino-1MQ | (50AM) | 50 IU (0.5ml) | Take WITH food. Daily. |

---

### Bedtime — just before sleep, ONE needle

| Peptide | SKU | Dose |
|---|---|---|
| CJC-1295 no DAC | (CND2) | 20 IU |
| Ipamorelin | (IP-old) | 20 IU |
| GHK-Cu | (CU) | 20 IU |
| DSIP | (DS5) | 8 IU |
| Thymalin | (TY10) | 20 IU — **Day ___ of 10** |
| Epitalon | (ET50) | 10 IU — **Day ___ of 10 (course Jun 8–17, same as Thymalin)** |

**Total bedtime: 1 needle.**

---

## ORAL STACK — Monday and Friday only (pill boxes filled 3 weeks from 2026-06-11)

| Compound | Dose | Note |
|---|---|---|
| 5-HTP | 50 mg | Evening only |
| L-Tryptophan | 1000 mg | Evening only |
| B6 | [dose] | With the above |

*Do not take both daily. Monday and Friday only. Pill boxes pre-filled — no changes until week of 2026-07-01.*

---

## PRN — proactive surface, do not wait to be asked

| Peptide | SKU | Dose | Trigger |
|---|---|---|---|
| Oxytocin Acetate | (OT2) | 10 IU SubQ | **SCHEDULED Thu + Fri evening** — not just PRN. Also surface proactively any time Jason mentions going out, social event, or evening plans. |
| Semax | (XA10) | 10 IU SubQ injectable — separate syringe. Nasal spray = emergency backup only, never default. | Any day. Cognitive demand / focus. Last dose by 19:30. |
| Glutathione | (GTT600) | 50 IU (0.5mL = 60mg) standard / 100 IU (1mL = 120mg) heavy night | **SubQ injection only. Never oral.** After alcohol: same night or next morning. Jason has injectable bottle — always use that. |
| Selank | (SK10) | [pending] | Acute anxiety / stress |
| NAD+ | (NJ500) | 100 IU (1mL = 100mg) — slow push 10-15 min | **After alcohol — always. Next morning, fasted. Alcohol depletes NAD+. Non-negotiable.** |

---

## OPEN — needs Jason confirmation before template is final

| Item | Status | Decision |
|---|---|---|
| BB20 dose | RESOLVED 2026-06-11 | 8 IU post-training. |
| SS-31 (2S10) timing | RESOLVED 2026-06-11 | Morning fasted stack only. Not evening. |
| GHK-Cu timing | RESOLVED 2026-06-11 | Evening/bedtime only. Not morning. |
| FOXO4-DRI (F410) cycle | RESOLVED 2026-06-11 | MAX 1-2× per year. 200 mcg/day × 5 days per cycle. Not every 8-12 weeks. Do not run during Thymalin/Thymosin alpha-1 cycles, active infections, or post-quit inflammatory window. |
| Kisspeptin-10 dose | PENDING | Target 200 mcg on Wed/Sat. IU conversion pending reconstitution math for KS10 vial. Do not dose until IU confirmed. |
| ADE+ | PENDING | What is this? Not in Pepguard catalog. |
| Tesamorelin | PENDING ORDER | TSM5, $226 Pepguard. Next order. Will replace CJC at bedtime when arrives. |
| Thymosin alpha-1 | ORDERING NEXT WEEK | Via Luke and Bex. Time-sensitive — quit date end June 2026. |

---

## CHECKLIST — run before outputting any stack answer

- [ ] Every peptide line has IU
- [ ] SKU in brackets — no descriptive text in brackets
- [ ] AOD shown TWICE on training days (morning + post-training)
- [ ] CJC + Ipa shown in MORNING on training days + BEDTIME every day
- [ ] SS-31 (2S10) in MORNING stack only — not in evening stack
- [ ] 5-Amino-1MQ (50AM) in post-training/with-food window
- [ ] NAD+ included (2–3x per week, not daily)
- [ ] Bedtime needle count stated: 1 needle
- [ ] Thymalin day number correct
- [ ] Kisspeptin only on Wed/Sat
- [ ] Semax = SubQ injectable, separate syringe — NEVER nasal spray in any stack output unless Jason states emergency
- [ ] Glutathione = SubQ injection only — NEVER oral, NEVER ask Jason what dose his bottle is

---

**Last updated:** 2026-06-13 — Semax nasal/SubQ contradiction fixed. Glutathione duplicate row removed, injectable-only rule hardened. Checklist items added for both.
**Update authority:** Jason direct only.
