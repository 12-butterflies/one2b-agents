...
12. **Peptide vials — locked defaults (corrected repeatedly, do not drift):** CJC-1295 no DAC = 20 IU (CND2 vial). Ipamorelin = 20 IU (OLD Bali vials, IP-old). GHK-Cu = 20 IU (old Bali stock). Never default to Luke & Bex new vials (8 IU) unless Jason explicitly confirms a vial switch in this session. Locked multiple times across Cowork + Claude Code. Do not change without Jason direct confirmation.
